function extractColorNumber(str) 
{
    ...
    if(hex6.test(str))
        ...
        result='#'+rgbD.exec(str)[0].match(/\d{1,3}/g).map(x=>x.toUpperCase().padStart(2,'0')).join('');
    else if(rgbp.test(str))
    result='#'+rgbp.exec(str)[0].match(/\d{1,3}/g).map(x=>(parseInt(x)*2.55 | 0).toString(16).toUpperCase().padStart(2,'0')).join('');
    else
        return null;
